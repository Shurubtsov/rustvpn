...
- `proxy` — VLESS+REALITY outbound to the VDS server (default outbound). When DPI bypass is enabled (default), the proxy outbound includes `sockopt.fragment` to split the TLS ClientHello into small TCP segments, defeating Russian TSPU deep packet inspection.
...
**`sockopt.fragment`** (DPI bypass) — When enabled, fragments the TLS ClientHello packet so Russian ISP DPI systems (TSPU) cannot reassemble and inspect the SNI field. This is critical for mobile internet in Russia where TSPU whitelists block access to Telegram, YouTube, Google, and other services. Parameters:
- `packets`: `"tlshello"` — only fragment TLS ClientHello packets (not all traffic)
- `length`: `"1-3"` — fragment size in bytes (tiny fragments defeat TSPU reassembly)
- `interval`: `"1-3"` — delay between fragments in milliseconds

Note: MUX is deliberately not used because it is incompatible with XTLS Vision flow — enabling MUX silently disables Vision, downgrading security. TLS fragment works alongside Vision and is the proven technique against TSPU.
